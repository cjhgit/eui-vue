<template>
    <div class="mu-pagination" v-if="total">
        ...
        <page-item v-if="totalPageCount > 5 && actualCurrent - 1 >= 4" identifier="backs" @click="handleClick"
                   title="前5页">
            <span>...</span>
        </page-item>
        <page-item v-for="item in pageList" :key="item" :index="item" @click="handleClick"
        ...
        <page-item :index="totalPageCount" @click="handleClick" :isActive="actualCurrent === totalPageCount"
                   v-if="totalPageCount !== 1"></page-item>
        ...
        <select-field v-if="showSizeChanger" v-model="actualPageSize" :style="{width: '100px'}">
            <menu-item v-for="item in pageSizeOption" :key="'mt_' + item" :value="item"
                       :title="item + pageSizeChangerText" :style="{width: '100px'}"/>
        </select-field>
