<template>
    <div @click.stop=""  :class="calendarClass" v-show="myShow" :style="calendarStyle" transition="calendar" transition-mode="out-in">
        <div  v-if="type!='time'">
...
export default {
    ...
    computed: {
        calendarStyle () {
            return  {
                left: this.placment === 'left' ? this.x + 'px' : '',
                right: this.placment !== 'left' ? this.x + 'px' : '',
                top: this.y + 'px'
            }
        },
        calendarClass () {
            return [
                'calendar',
                {
                    'right': this.placment === 'right',
                    'left': this.placment === 'left'
                }
            ]
        }
    },
    ...
    methods: {
        ...
        init (){
            var now = new Date();
            if (this.myValue != "") {
                if (this.myValue.indexOf("-") != -1) this.mySep = "-"
                if (this.myValue.indexOf(".") != -1) this.mySep = "."
                if (this.myValue.indexOf("/") != -1) this.mySep = "/"
                if (this.type == "date") {
                    var split = this.myValue.split(this.mySep)
                    this.year = parseInt(split[0])
                    this.month = parseInt(split[1]) - 1
                    this.day = parseInt(split[2])
                } else if (this.type == "datetime") {
                    var split = this.myValue.split(" ")
                    var splitDate = split[0].split(this.mySep)
                ...
                if (this.myRange) {
                    var split = this.myValue.split(" ~ ")
                    if (split.length > 1) {
                        var beginSplit = split[0].split(this.mySep)
                        var endSplit = split[1].split(this.mySep)
        ...
        render (y, m) {
            ...
            var seletSplit = this.myValue.split(" ")[0].split(this.mySep)
            var i, line = 0,temp = []
                ...
                if (this.myRange) {
                    ...
                    if (
                     ...
                     // 没有默认值的时候显示选中今天日期
                        ...
                        if (this.myBegin != "") {
                            var beginSplit = this.myBegin.split(this.mySep)
                        ...
                        if (this.myEnd != ""){
                            var endSplit = this.myEnd.split(this.mySep)
        ...
        select (k1, k2, e) {
            ...
            if (this.myRange) {
                ...
                this.today = [k1, k2]
                if (this.type == 'date') {
                    this.myValue = this.year + this.mySep + this.zero(this.month + 1) + this.mySep + this.zero(this.days[k1][k2].day)
                    this.myShow = false
                }
            }
        },
        ...
        ok () {
            ...
            if (this.myRange) {
                this.myValue = this.output(this.rangeBegin) + " ~ " + this.output(this.rangeEnd)
            } else {
                this.myValue = this.output([
                    this.year,
                    this.month, 
                    this.day,
                    parseInt(this.hour),
        ...
        output (args) {
            ...
            if (this.type == 'datetime') {
                return args[0] + this.mySep + this.zero(args[1] + 1) + this.mySep + this.zero(args[2]) + " " + this.zero(args[3]) + ":" + this.zero(args[4]) + ":" + this.zero(args[5])
            }
            if (this.type == 'date') {
                return args[0] + this.mySep + this.zero(args[1] + 1) + this.mySep + this.zero(args[2])
            }
        }
    }
}
